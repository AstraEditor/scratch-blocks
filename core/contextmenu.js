...
/**
 * 关于局部加速模式的菜单，这会使脚本不再延迟，就跟“加速模式”一样
 * 下面不知道干什么的，总之复制就对了（好像是关于语法高亮什么的？）
 * @param {!Blockly.WorkspaceSvg} ws The workspace where the right-click
 *     originated.
 * @param {number} numTopBlocks The number of top blocks on the workspace.
 * @return {!Object} A menu option, containing text, enabled, and a callback.
 * @package
 */
Blockly.ContextMenu.partialTurboMode = function (block) {
  var isHatBlock = !block.outputConnection && !block.previousConnection;
  var existingComment = block.getCommentText();
  var isEnabled = existingComment && existingComment.indexOf('[Turbo]') !== -1;
  return {
    text: isEnabled === 'false' || !isEnabled
      ? Blockly.Msg.ENABLE_PARTIAL_TURBOMODE
      : Blockly.Msg.UNABLE_PARTIAL_TURBOMODE,
    enabled: isHatBlock,
    callback: function () {
      if (isEnabled) {
        block.setCommentText(null);
      ...
      if (typeof vm !== 'undefined' && vm.setProjectChanged) {
        vm.setProjectChanged();
      }
    }
  }
};
